...
import { CheckCircle2, Calendar, Clock, User, Scissors, Mail, MessageCircle } from "lucide-react";
...
export default function BookingConfirmationModal({
    ...
    return (
        <Modal
            ...
            isKeyboardDismissDisabled={isSuccess}
            classNames={{
                    ...
                    <div className="relative flex flex-col w-full h-full">
                        ...
                        <ModalHeader className="flex flex-col gap-4 items-center justify-center pt-10 pb-2 bg-transparent">
                            <div className="relative group">
                                <div className="absolute -inset-4 bg-[#D09E1E]/20 rounded-full blur-xl opacity-50 group-hover:opacity-100 transition-opacity duration-500" />
                                <div className="relative w-20 h-20 rounded-[1.5rem] bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center border border-white/10 shadow-[inset_0_0_20px_rgba(255,255,255,0.05)]">
                                    <Scissors className="text-[#D09E1E] drop-shadow-[0_2px_10px_rgba(208,158,30,0.5)]" size={32} />
                                </div>
                            </div>
                        ...
                        <ModalBody className="px-6 py-4 gap-4 overflow-y-auto custom-scrollbar">
                            <div className="space-y-3">
                                <span className="text-[9px] font-black text-[#D09E1E] uppercase tracking-[0.2em] ml-2">Servicios Seleccionados</span>
                                {bookingData.services.map((service, idx) => (
                                    <div key={idx} className="p-4 rounded-[1.5rem] bg-white/5 border border-white/5 flex flex-col gap-1 relative overflow-hidden group">
                                        <div className="absolute inset-0 bg-gradient-to-br from-[#D09E1E]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                                        <h3 className="text-sm font-bold text-white leading-tight">{service.name}</h3>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-white/60 text-[11px] font-bold font-mono">${service.price}</span>
                            ...
                            {bookingData.services.length > 1 && (
                                <div className="px-6 py-3 rounded-2xl bg-[#D09E1E]/10 border border-[#D09E1E]/20 flex justify-between items-center">
                                    <span className="text-[10px] font-black text-[#D09E1E] uppercase tracking-widest">Total Estimado</span>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <InfoTile icon={<User size={16} />} label="Barbero" value={bookingData.barberName} />
                                <InfoTile icon={<Calendar size={16} />} label="Fecha" value={bookingData.date} />
                                <InfoTile icon={<Clock size={16} />} label="Hora" value={bookingData.time} className="col-span-2 flex-row items-center gap-4 px-5" />
                            </div>
                            ...
                            {isSuccess ? (
                                <div className="w-full flex flex-col items-center gap-6 py-4 animate-in fade-in zoom-in duration-300">
                                    ...
                                    <div className="text-center space-y-4 px-2">
                                        ...
                                        <div className="p-4 rounded-[1.5rem] bg-amber-500/10 border border-amber-500/20 text-center">
                                            <p className="text-amber-200 text-[11px] font-medium leading-relaxed">
                                                Tu cita está en estado <span className="font-black uppercase tracking-tighter">preagendada</span> hasta que sea confirmada.
                                            </p>
                                        </div>
                                        <div className="p-4 rounded-[1.5rem] bg-blue-500/10 border border-blue-500/20 text-center">
                                            <p className="text-blue-200 text-[11px] font-medium leading-relaxed">
                                                Para formalizar la reserva, envía el comprobante de pago de <span className="font-black">$3</span> (adicionales al servicio).
                                            </p>
                                        </div>
                                        <a
                                            href="https://wa.me/50762146379"
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="w-full h-14 rounded-[1.5rem] bg-[#25D366] text-black font-black uppercase tracking-tight text-sm flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 transition-all shadow-[0_10px_30px_-5px_rgba(37,211,102,0.3)] mt-2"
                                        >
                                            {/* Usamos FaWhatsapp en lugar de MessageCircle */}
                                            <FaWhatsapp size={24} />
                                            Enviar Comprobante
                                        </a>
                                    </div>
                                    <Button
                                        className="w-full h-12 rounded-[1.2rem] bg-white/5 text-white/60 font-bold uppercase tracking-widest text-[10px] hover:bg-white/10 border border-white/5 transition-all mt-2"
                                ...
                                <>
                                    {(isGuest || !bookingData.clientEmail || bookingData.clientEmail === "N/A") && (
                                        <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 flex justify-center mb-2">
                                            <p className="text-red-400 text-[10px] font-bold uppercase tracking-widest leading-relaxed text-center">
                                                {isGuest ? "Inicia sesión con Google para agendar tu cita" : "Inicia sesión para agendar"}
                                            </p>
                                        </div>
                                    )}
                                    <div className="flex gap-3 w-full">
                                        ...
                                        {!isGuest && (
                                            <Button
                                                ...
                                                style={{ background: 'linear-gradient(135deg, #D09E1E 0%, #D09E1E 100%)' }}
                                                onPress={onConfirm}
...
const InfoTile = ({ icon, label, value, className = "" }: { icon: React.ReactNode, label: string, value: string, className?: string }) => (
    <div className={`p-4 rounded-[1.5rem] bg-white/[0.03] border border-white/5 flex flex-col justify-center items-start gap-2 hover:bg-white/[0.06] transition-colors ${className}`}>
        <div className="text-[#D09E1E] opacity-80">{icon}</div>
        <div>
